...
if(!$bid||!$sid){ header('Location: batches.php'); exit; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Topics · 𝑫𝑨𝑹𝑲 𝑼𝑵𝑰𝑽𝑬𝑹𝑺𝑬</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
<style>
*{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
:root{--p:#7c3aed;--p2:#a78bfa;--g:#06b6d4;--g2:#22d3ee;--bg:#020208;--c1:#0b0b12;--c2:#10101a;--b1:#1c1c28;--b2:#252535;--t1:#f1f5f9;--t2:#94a3b8;--t3:#475569;--t4:#334155}
...
.tb{flex-shrink:0;height:64px;display:flex;align-items:center;padding:0 16px;gap:12px;background:rgba(2,2,8,.94);border-bottom:1px solid rgba(6,182,212,.1);backdrop-filter:blur(24px);z-index:100}
...
.crumb{display:flex;align-items:center;gap:6px;padding:12px 16px;overflow-x:auto;flex-nowrap:nowrap}
.crumb::-webkit-scrollbar{display:none}
.crumb a{font-size:9px;color:var(--t4);font-family:'JetBrains Mono',monospace;letter-spacing:.08em;white-space:nowrap;cursor:pointer;flex-shrink:0}
.crumb a.act{color:var(--g2);font-weight:700}
.crumb-sep{color:var(--b2);font-size:9px;flex-shrink:0}
.hstrip{padding:12px 16px 0}
.hstrip-title{font-size:20px;font-weight:900;letter-spacing:-.02em;margin-bottom:4px}
.hstrip-title em{font-style:normal;background:linear-gradient(135deg,#22d3ee,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
...
.srch input:focus{border-color:rgba(6,182,212,.5);background:var(--c2)}
.srch input::placeholder{color:var(--t4)}
.sh{display:flex;align-items:center;padding:16px 16px 12px}
.sh-left{display:flex;align-items:center;gap:8px;flex:1}
.sh-ico{width:30px;height:30px;border-radius:10px;background:rgba(6,182,212,.08);border:1px solid rgba(6,182,212,.18);display:flex;align-items:center;justify-content:center;font-size:12px;color:var(--g2)}
...
.lcard::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(to bottom,var(--g),var(--p));border-radius:3px 0 0 3px;opacity:0;transition:opacity .2s}
.lcard:active{background:var(--c2);border-color:rgba(6,182,212,.28);transform:scale(.985)}
.lcard:active::before{opacity:1}
.lico{width:46px;height:46px;border-radius:14px;background:rgba(6,182,212,.07);border:1px solid rgba(6,182,212,.16);display:flex;align-items:center;justify-content:center;font-size:18px;color:var(--g2);flex-shrink:0}
...
.spin{width:34px;height:34px;border:2.5px solid var(--b1);border-top-color:var(--g);border-radius:50%;animation:spin .75s linear infinite}
...
.retry{margin-top:6px;background:rgba(6,182,212,.1);border:1px solid rgba(6,182,212,.25);color:var(--g2);padding:11px 24px;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif}
...
<div id="app">
  <div class="tb">
    ...
    <div class="tb-info">
      <div class="tb-title"><?=htmlspecialchars($sn)?></div>
      <div class="tb-sub">TOPICS CHOOSE KARO</div>
    </div>
  ...
  <div class="scroll">
    <div class="crumb">
      ...
      <a onclick="history.go(-1)"><?=htmlspecialchars($bn)?></a>
      <span class="crumb-sep"><i class="fas fa-chevron-right"></i></span>
      <a class="act"><?=htmlspecialchars($sn)?></a>
    </div>
    <div class="hstrip">
      <div class="hstrip-title">Choose <em>Topic</em></div>
      <div class="hstrip-sub" id="hs">Topics load ho rahe hain…</div>
    </div>
    <div class="srch-wrap">
      <div class="srch"><i class="srch-i fas fa-search"></i>
        <input type="text" placeholder="Topic search karo…" id="srch" oninput="filter(this.value)" autocomplete="off">
      </div>
    </div>
    <div class="sh">
      <div class="sh-left"><div class="sh-ico"><i class="fas fa-lightbulb"></i></div><div class="sh-txt">Topics</div></div>
      <div class="sh-cnt" id="cnt">…</div>
    </div>
    <div class="list" id="list">
      <div class="ld-wrap"><div class="spin"></div><div class="ld-txt">TOPICS LOAD HO RAHE HAIN…</div></div>
    </div>
  </div>
  <nav class="bnav">
    <div class="ni" onclick="window.location.href='batches.php'"><i class="fas fa-home"></i><span>HOME</span></div>
    <div class="ni on"><i class="fas fa-lightbulb"></i><span>TOPICS</span></div>
...
<div class="toast" id="toast"><i class="fas fa-circle-info" style="color:var(--g2)"></i><span id="tmsg"></span></div>
...
const BID=<?=intval($bid)?>,BN=<?=json_encode(urldecode($_GET['bn']??'Batch'))?>,SID=<?=intval($sid)?>,SN=<?=json_encode(urldecode($_GET['sn']??'Subject'))?>;
let _all=[];
...
async function load(){
  document.getElementById('list').innerHTML='<div class="ld-wrap"><div class="spin"></div><div class="ld-txt">TOPICS LOAD HO RAHE HAIN…</div></div>';
  const r=await rApi('topics',{bid:BID,sid:SID});
  _all=(r.data||[]).sort((a,b)=>(a.topicid||0)-(b.topicid||0));
  if(!_all.length){document.getElementById('list').innerHTML=`<div class="empty-wrap"><i class="fas fa-satellite-dish"></i><h3>Topics nahi aaye</h3><p>${H(r.error||'Network error')}</p><button class="retry" onclick="load()"><i class="fas fa-rotate-right"></i> Retry</button></div>`;document.getElementById('cnt').textContent='ERR';return;}
  document.getElementById('hs').textContent=_all.length+' topics available';
  render(_all);
}
function filter(q){render(q?_all.filter(t=>(t.topic_name||'').toLowerCase().includes(q.toLowerCase())):_all);}
function render(list){
  document.getElementById('cnt').textContent=list.length+' TOTAL';
  const el=document.getElementById('list');
  if(!list.length){el.innerHTML='<div class="empty-wrap"><i class="fas fa-folder-open"></i><h3>Koi topic nahi mila</h3></div>';return;}
  el.innerHTML=list.map((t,i)=>`<div class="lcard" onclick="go(${t.topicid})" style="animation-delay:${i*.04}s">
    <div class="lico"><i class="fas fa-lightbulb"></i></div>
    <div class="ltxt"><div class="lname">${H(t.topic_name)}</div><div class="lsub">${t.content_count?t.content_count+' ITEMS':'TAP TO OPEN'}</div></div>
    <i class="fas fa-chevron-right larr"></i>
  </div>`).join('');
}
function go(tid){const t=_all.find(x=>x.topicid==tid);window.location.href='content.php?bid='+BID+'&bn='+encodeURIComponent(BN)+'&sid='+SID+'&sn='+encodeURIComponent(SN)+'&tid='+tid+'&tn='+encodeURIComponent(t?.topic_name||'Topic');}
